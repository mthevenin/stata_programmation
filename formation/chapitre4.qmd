...
| **Ouverture et sauvegarde d'une base**                     | `use` `webuse [set]` `sysuse` `save` `saveold` `import` `export` **`usesas`** **`savasas`** |
| **Décrire le contenu d'une base**                          | `describe` `list` `codebook` `labelbook` `label list`                                       |
| **Commandes utiles**                                       | `sort` `gsort` `duplicates` `order`                                                         |
| **Description statistique d'une base**                     | `summarize` `mean` `tabstat` `tabulate` `fre` `tabm`                                        |
...
# **Tri, doublon, déplacement des variables**
...
sort price tri croissant sur la variable price
...
Juste une présentation de quelques visualisation permettant d'explorer des distributions.

Une seule distribution:   **boxplot**, **histogram**, **`violinplot`** (externe)  
