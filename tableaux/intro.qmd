...
-   Le tableau a été compilé avec **`overleaf`** (https://fr.overleaf.com), éditeur Latex en ligne.
-   le fichier tex1.tex généré avec `collect export` donne le tableau brut balisé. Le fichier peut être appelé t inséré dans un document Latex avec `\estauto{nom_fichier.tex}`
